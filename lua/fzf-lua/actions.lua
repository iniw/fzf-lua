...
M.normalize_selected = function(actions, selected, opts)
  ...
  if not actions or not selected then
    return
  end
...
M.vimcmd_entry = function(_vimcmd, selected, opts, pcall_vimcmd)
  for i, sel in ipairs(selected) do
    (function()
      ...
      if
        vimcmd == "e" and (entry.uri or path.equals(fullpath, CTX.bname))
      ...
      then
        vim.print(
          string.format(
            "Overriding window: %s",
            string.format(
              "will_replace = %s, hidden = %s, writeall = %s, dirty = %s",
              will_replace_curbuf,
              vim.o.hidden,
              vim.o.autowriteall,
              utils.buffer_is_dirty(nil, false, true)
            )
          )
        )
